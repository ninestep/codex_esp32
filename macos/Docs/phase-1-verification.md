...
- Queue files and lock files are opened with `O_CREAT|O_RDWR|O_NOFOLLOW` and owner-only `0600` validation; the socket parent still uses `SocketParentPreparer` strict `0700` owner checks. Enqueue and drain serialize through `flock`. The first pending event that returns `.error` and all later events are retained. The queue file is not deleted on service exit.
