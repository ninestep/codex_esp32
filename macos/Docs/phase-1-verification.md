...
Final-review remediation was applied on 2026-08-03 for the High/Medium findings from the Phase1 review and follow-up quality review:
...
- High: the pending queue now stores internal versioned `PendingLocalEvent` frames, with two event types: `.launchSnapshot(LaunchRegistration)` and `.hook(HookPayload)`. When `register-launch` cannot connect before sending the request, the helper captures the focused Ghostty terminal immediately in that same helper invocation, builds a trusted `LaunchRegistration` snapshot, enqueues it, and exits 0 with fixed stderr `codex-remote-helper: launch queued`. `serve` drains queued launch snapshots before later hooks in file order, so a queued launch snapshot followed by `SessionStart` restores launcher/provider/terminal mapping without re-capturing focus later.
- High: startup IPC requests are gated after `server.start` and before dispatcher handling until pending queue drain finishes. The socket handler waits on `IPCStartupGate`, while pending events drain directly through `SessionIPCDispatcher`; after drain succeeds or fails, the helper explicitly opens the gate so startup requests cannot bypass queued launch snapshots and cannot remain blocked forever on a drain error.
- High: queueable transport errors are now limited to pre-send connection failures only: `connectFailed` and `connectTimedOut`. Hook and register-launch do not queue after send/write/read ambiguity, empty response, read timeout, local codec failure, or explicit daemon negative responses; those paths return 69 to avoid duplicate or at-most-once violations.
- Medium: queue rewrites now use crash-safer atomic replacement instead of `ftruncate` in-place writes. While holding the stable lock file, the helper reads the old queue, writes complete new frames to a unique same-directory temp file opened with `O_CREAT|O_EXCL|O_NOFOLLOW` and `0600`, fsyncs the temp file, renames it over `pending-hooks.jsonl`, then fsyncs the parent directory. Failures before rename unlink the temp file and leave old queue bytes unchanged. If parent directory fsync fails after rename, the code reports an error but cannot roll back the already-renamed file; this is not claimed as an absolute transaction.
- Queue persistence stores only normalized pending event fields. Hook `message` and `lastAssistantMessage` are truncated to 1,024 Unicode characters each; no raw hook JSON or transcript is persisted. Queue bounds remain 64 events, 256 KiB total file size, and 64 KiB per frame, dropping oldest events first.
- The lock file is opened with `O_CREAT|O_RDWR|O_NOFOLLOW` and owner-only `0600` validation. Existing queue files are read with `O_RDONLY|O_NOFOLLOW` after the same owner/type/mode checks. Queue rewrites use a unique same-directory temp file opened with `O_CREAT|O_EXCL|O_RDWR|O_NOFOLLOW` and `0600`, then fsync the temp file, rename it over `pending-hooks.jsonl`, and fsync the parent directory. The socket parent still uses `SocketParentPreparer` strict `0700` owner checks. Enqueue and drain serialize through `flock`. The first pending event that returns `.error` and all later events are retained. The queue file is not deleted on service exit.

Focused remediation tests:

| Check | Command | Exit | Evidence |
| --- | --- | ---: | --- |
| Ghostty bundle id plus pending queue behavior | `cd macos && swift test --filter 'GhosttyAppleScriptControllerTests\|HookEventQueueTests'` | 0 | 27 tests passed. Tests cover capture/focus/scroll/sendKey bundle id scripts, hook queueing, launch snapshot queueing, SessionStart retention without launch, launchSnapshot + SessionStart drain mapping recovery, bounded queue dropping, file mode and symlink rejection, drain success/failure retention, concurrent enqueue, and atomic rewrite failure before rename. |
| Helper command and queue regression | `cd macos && swift test --filter 'HelperCommandTests\|HookEventQueueTests'` | 0 | 46 tests passed, covering raw hook parse exit 65, explicit daemon errors exit 69, connectFailed/connectTimedOut queueing, post-send failures not queued, non-hook command behavior, IPC server/client behavior, pending event drain, and atomic rewrite semantics. |
| Startup gate ordering | `cd macos && swift test --filter 'IPCStartupGateTests'` | 0 | 3 tests passed, covering wait-before-open, idempotent open releasing all waiters, direct `SessionStart` waiting until pending launch snapshot drain restores mapping, and drain-error open releasing waiters. |
| Socket connection limit stability | `cd macos && swift test --filter 'UnixSocketEventServerTests/testConnectionLimitReturnsServerBusyAndKeepsExistingServerUsable\|UnixSocketEventServerTests/testBlockingEventGateWaitForBlockedCountTimesOutAndReturnsAfterTargetReached'` | 0 | After the flaky failure and follow-up Medium review fix, both target tests passed once, then passed 20 consecutive reruns. The connection-limit test synchronizes on two handler-blocked slot occupants before asserting `server_busy`, retries only temporary `read` EAGAIN/EWOULDBLOCK in eventual socket helpers, retries EINTR inside `readUntilEOF`, registers async teardown to release the slot gate, and releases both occupied slots before verifying a later valid frame succeeds. The gate test proves an unmet count throws `TestTimeoutError.timedOut` with a short polling deadline instead of hanging, and a reached count returns normally. |

Final remediation fresh verification:

| Check | Command | Exit | Evidence |
| --- | --- | ---: | --- |
| Full Swift test suite, run 1 after socket gate timeout fix | `cd macos && swift test --parallel` | 0 | Build completed in `0.15s`; XCTest enumerated `[1/118]` through `[118/118]` with no failures. Swift Testing compatibility summary printed `Test run with 0 tests in 0 suites passed`. |
| Full Swift test suite, run 2 after socket gate timeout fix | `cd macos && swift test --parallel` | 0 | Build completed in `0.12s`; XCTest enumerated `[1/118]` through `[118/118]` with no failures. Swift Testing compatibility summary printed `Test run with 0 tests in 0 suites passed`. |
| Swift build | `cd macos && swift build` | 0 | `Build complete! (0.18s)`. |
...
| Final fresh socket connection-limit flaky failure | First final fresh `cd macos && swift test --parallel` after commit `bfca4f7` failed only `UnixSocketEventServerTests/testConnectionLimitReturnsServerBusyAndKeepsExistingServerUsable` with POSIX `read` errno 35 (`EAGAIN`). The target then passed repeated runs, showing a timing-sensitive test helper failure rather than production server evidence. During repair, intermediate reruns reproduced the same errno when busy/slot occupancy was still inferred from connected idle clients. | Test-only repair: `sendFrameEventually` now retries only temporary `read` EAGAIN/EWOULDBLOCK, `readUntilEOF` retries EINTR, and the connection-limit test now uses two handler-blocked slot occupants so `server_busy` is asserted only after the server has definitely consumed both active slots. The test then releases both occupants before verifying a later valid frame succeeds. | Fixed: target test passed once and then 20 consecutive reruns; full `swift test --parallel` passed twice with final XCTest count `[1/117]` through `[117/117]`; `swift build` and shim script also exited 0. |
| Follow-up Medium review on slot gate count wait | Review found `BlockingEventGate.waitForBlockedCount` stored checked continuations without an explicit timeout, so a missed count could hang the test instead of failing clearly. | Test-only repair: removed count wait continuations, changed `waitForBlockedCount` to actor-reentrant polling with `ContinuousClock`, 10ms default poll interval, explicit timeout throwing `TestTimeoutError.timedOut`, and natural cancellation propagation. The connection-limit test registers async teardown that always releases handler waiters; normal-path explicit release remains idempotent. Added a gate test covering timeout and reached-count success. | Fixed: the two target tests passed once and then 20 consecutive reruns; full `swift test --parallel` passed twice with final XCTest count `[1/118]` through `[118/118]`; `swift build` and shim script also exited 0. |
