...
| Socket connection limit stability | `cd macos && swift test --filter 'UnixSocketEventServerTests/testConnectionLimitReturnsServerBusyAndKeepsExistingServerUsable'` | 0 | After the flaky failure fix, the target test passed once, then passed 20 consecutive reruns. The test now synchronizes on two handler-blocked slot occupants before asserting `server_busy`, retries only temporary `read` EAGAIN/EWOULDBLOCK in eventual socket helpers, retries EINTR inside `readUntilEOF`, and releases both occupied slots before verifying a later valid frame succeeds. |

Final remediation fresh verification:

| Check | Command | Exit | Evidence |
| --- | --- | ---: | --- |
| Full Swift test suite, run 1 after socket flaky fix | `cd macos && swift test --parallel` | 0 | Build completed in `0.15s`; XCTest enumerated `[1/117]` through `[117/117]` with no failures. Swift Testing compatibility summary printed `Test run with 0 tests in 0 suites passed`. |
| Full Swift test suite, run 2 after socket flaky fix | `cd macos && swift test --parallel` | 0 | Build completed in `0.10s`; XCTest enumerated `[1/117]` through `[117/117]` with no failures. Swift Testing compatibility summary printed `Test run with 0 tests in 0 suites passed`. |
| Swift build | `cd macos && swift build` | 0 | `Build complete! (0.16s)`. |
