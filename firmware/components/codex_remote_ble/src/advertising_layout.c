...
enum {
    ...
    APPEARANCE_FIELD_LENGTH = 4,
    UUID16_FIELD_LENGTH = 4,
...
{
    const size_t primary_length = FLAGS_FIELD_LENGTH
        + APPEARANCE_FIELD_LENGTH
        + UUID16_FIELD_LENGTH;
    const size_t scan_response_length = FIELD_HEADER_LENGTH
        + name_length
        + UUID128_FIELD_LENGTH;
    return (cr_ble_advertising_layout_t) {
        .primary_length = primary_length,
        .scan_response_length = scan_response_length,
        .name_in_scan_response = true,
