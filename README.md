...
设备烧录、BlackHole 安装、系统权限申请以及用户级 hooks 或 shell 配置修改均属于现场操作，不会由普通仓库构建自动执行。
...
自动化测试只能验证协议、状态机、配置流程和可构建性。屏幕、触控、GPIO18、麦克风、BLE 时序、功耗及完整语音链路必须在真实设备上验收。现场安装和联调步骤见 [设备到位启用手册](docs/设备到位启用手册.md)。
